...
# get all
@router.get("/", response_model=list[staffResponse], dependencies=[Depends(require_position(staffPosition.admin, staffPosition.manager))])
def get_all_staff(db:Session = Depends(database.getDB)):
    return staff_service.get_all_staff(db)

# get by ID
@router.get("/{id}", response_model=staffResponse, dependencies=[Depends(require_position(staffPosition.admin, staffPosition.manager))])
def get_staff_by_id(id:int, db:Session = Depends(database.getDB)):
    return staff_service.get_all_staff(id, db)

# deactivate
@router.patch("/deactivate/{id}", response_model=staffResponse, dependencies=[Depends(require_position(staffPosition.admin, staffPosition.manager))])
...
# register
@router.post("/register", response_model=staffResponse, dependencies=[Depends(require_position(staffPosition.admin, staffPosition.manager))])
def register_staff(request:staffRegister, db:Session = Depends(database.getDB)):
    return staff_service.register_staff(db, request)

@router.patch("/{id}", response_model=staffResponse, dependencies=[Depends(require_position(staffPosition.admin, staffPosition.manager))])
